...
public abstract class AbstractBinaryTree<T> implements BinaryTree<T> {
	...
	public Iterator<Position<T>> positions() {
		// create anonymous class from Iterator
		return new Iterator<Position<T>>() {
			Iterator<Position<T>> iterator;
			
			// create anonymous  visitor within anonymous iterator
			//this does nothing other than visit the children, no other interaction takes place
			...
			//We are still inside the anonymous class body of visitor
			// Static initialization block - invokes the visit() method on the
			// root node, which will recursively visit every other node. 
			//This is not a static initialization block it is just a normal instance initialization block?!
			{
				List<Position<T>> list = new LinkedList<Position<T>>();
	...
	public T replace(Position<T> p, T t) {
		// why am I casting p rather than getting the element it points to?
		// why not just invoke the positions element() function?
		// T temp = p.element().element();
		// p.element() = T;?
	...
	public String toString() {
		...
		// Seriously in-line anonymous class :o)
		new Visitor<T>() {
